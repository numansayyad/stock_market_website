...
//         // Redirect to dashboard with user data in URL (since different ports don't share localStorage)
//         const userName = data.name || data.email.split('@')[0];
//         window.location.replace(`http://localhost:3000?userId=${data.userId}&userName=${encodeURIComponent(userName)}&userEmail=${encodeURIComponent(data.email)}`);
//         return;
    ...
    try{
      ...
      if(response.ok){
        ...
        window.location.replace(
          `http://localhost:3000?userId=${data.userId}&userName=${encodeURIComponent(userName)}&userEmail=${encodeURIComponent(data.email)}`
        );
...
<img src="/media/account_open.svg" alt="login" className="login-left-img"/>
